...
lista_de_especies <-
  unique(captura_puerto_flota_2010_2018$especie) |> view()
```
...
lista_de_puertos <-
  unique(captura_puerto_flota_2010_2018$puerto) |> view()
...
captura_puerto_flota_2010_2018 |>
  group_by(puerto) |>
  mutate(total_puerto = sum(captura)) |> 
  select(puerto, captura, especie, total_puerto) |> 
  filter(puerto == "Caleta Cordova") |>
  group_by(especie) |>
...
```{r}
tabla1 |> 
  mutate(captura_total= sum(total_especie)) |> 
