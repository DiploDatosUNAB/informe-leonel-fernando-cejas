...
ggplot(data=especies_repre)

especies_repre |>
  ggplot (especies_repre, aes (fecha, captura)) +
  geom_point()
```
...
## ensanchar la tabla con las especies
Quiero tener la especie en cada columna
...
```{r}
tabla_especies <- captura_puerto_flota_2010_2018 |> 
  pivot_wider(names_from = especie, values_from = captura, values_fill = 0) |> 
  arrange(fecha)
